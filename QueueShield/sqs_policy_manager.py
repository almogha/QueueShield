...
def update_external_access(statements):
    ...
    internal_principals = f"arn:aws:iam::{AWS_ACCOUNT_ID}:root"
    for statement in statements:
        principal = statement.get("Principal", {})

        if "*" in principal or principal.get("AWS") != internal_principals:
...
def process_queues_in_all_regions():
    ...
    if failed_regions:
        ...
        raise Exception("Failed to process queues in some regions")
